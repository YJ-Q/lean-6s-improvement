...
- 单条问题：先给 1 条可派工结果，再补关键追问。
- 多条问题：默认切换为“批量台账模式”，一条问题对应台账中的一行。
- 只有用户明确要求时，才展开成长版分析、完整整改单或闭环审核说明。

这能减少管理者把 AI 输出重新手工整理进 Excel、群消息或整改台账的工作量。
...
如果你一天会收集多条现场问题，建议直接把问题清单一次发给 AI。skill 应默认按“批量台账模式”输出，而不是逐条长篇分析。

推荐输出列：

```text
序号 | 区域/点位 | 问题简述 | 主分类 | 风险等级 | 当班动作 | 责任角色 | 完成时限 | 验收证据 | 是否需追问
```

推荐提问方式示例：

```text
以下是今天现场问题，请按批量台账模式整理，逐条输出成可直接复制到整改台账的表格，不要展开长分析：
1. ...
2. ...
3. ...
```
